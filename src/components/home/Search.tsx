import { View, Text, Alert, Touchable, TouchableOpacity, Image } from 'react-native'
...
const Search = () => {
  ...
  const [locationType, setLocationType] = React.useState<'from' | 'to'>('from');
  ...
  const handleSearchBuses = () => {
    if (!from || !to) {
      Alert.alert('Missing Information', 'Please select both departure and arrival locations');
      return;
    }
    if (from === to) {
      Alert.alert('Invalid Search', 'Departure and arrival locations cannot be the same');
      return;
    }
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    if (date < today) {
      Alert.alert('Invalid Search', 'Please select a future date for your trip');
      return;
    }
  ...
  return (
    <View className='rounded-b-3xl overflow-hidden'>
      ...
      >
        <View className='p-4'>
          <View className='my-4 border border-1 z-20 bg-white rounded-md border-gray-600'>
            <TouchableOpacity className='p-4 flex-row gap-4 items-center' onPress={() => { setLocationType('from'), setShowLocationPicker(true) }}>
              <Image source={require('../../assets/images/bus.png')} className='h-6 w-6' />
              <Text className='w-[90%] text-lg font-okra text-gray-600'>
                {from || 'From'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity className='p-4 border-t-[1px] border-b-[1px]  border-gray-400 flex-row gap-4 items-center' onPress={() => { setLocationType('to'), setShowLocationPicker(true) }}>
              <Image source={require('../../assets/images/bus.png')} className='h-6 w-6' />
              <Text className='w-[90%] text-lg font-okra text-gray-600'>
                {to || 'To'}
              </Text>
            ...
            <View className='flex-row items-center p-2 justify-between'>
              ...
              >
                <View className='mr-3 '>
                  <Text className='font-bold text-sm font-okra text-gray-500'>Date of Journey</Text>
                  <Text className='font-bold text-md font-okra text-gray-700'>{date?.toDateString()}</Text>
                </View>
      ...
      {showDatePicker && <DatePickerModal visible={showDatePicker} onClose={() => setShowDatePicker(false)} onConfirm={setDate} selectedDate={date} />}
